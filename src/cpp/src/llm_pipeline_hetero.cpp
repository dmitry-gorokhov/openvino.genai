...
#include "llm_pipeline_hetero.hpp"
...
HeteroLLMPipeline::HeteroLLMPipeline(
...
HeteroLLMPipeline::HeteroLLMPipeline(
    ...
    ov::CompiledModel compiled_model_generate;
    if (auto filtered_properties = extract_adapters_from_properties(properties, &m_generation_config.adapters)) {
        ...
        // compiled_model_prefill = utils::singleton_core().compile_model(model, device, *filtered_properties);
    ...
    ov::genai::utils::print_compiled_model_properties(compiled_model_prefill, "Hetero LLM model (prefill)");
    ov::genai::utils::print_compiled_model_properties(compiled_model_generate, "Hetero LLM model (generate)");
...
HeteroLLMPipeline::HeteroLLMPipeline(
    ...
    : HeteroLLMPipeline{models_path, Tokenizer(models_path), device, plugin_config} {}

DecodedResults HeteroLLMPipeline::generate(
    StringInputs inputs,
    OptionalGenerationConfig generation_config,
...
EncodedResults HeteroLLMPipeline::generate(
    ...
    auto num_inputs = m_model_runner_prefill.get_compiled_model().inputs().size();
    ...
    if (m_kv_history_manager.reset_kv_cache)
        ...
        ov::genai::utils::trim_kv_cache(m_model_runner_prefill, m_kv_history_manager.num_tokens_to_remove_from_kv_cache,
    ...
    if (is_chat_conversation && !m_tokenized_chat_history.empty()) {
        ...
        auto atten_mask_history = m_model_runner_prefill.get_tensor("attention_mask");
    ...
    if(m_adapter_controller) {
        m_adapter_controller->apply(m_model_runner_prefill, config.adapters);
    }
    ...
    ov::genai::utils::GenerationFinishInfo finish_info = get_lm_encoded_results(m_model_runner_prefill, m_model_runner_generate, input_ids, concatenated_attention_mask,
    ...
    if (is_chat_conversation) {
        ...
        if (config.is_beam_search() && m_chat_input_type != ov::genai::utils::GenerationChatInputsType::ENCODED_INPUTS) {
            ...
            m_kv_history_manager.num_tokens_to_remove_from_kv_cache = m_model_runner_generate.get_tensor("attention_mask").get_shape()[1] - prev_attn_mask_size;
        }
...
void HeteroLLMPipeline::start_chat(const std::string& system_message) {
...
void HeteroLLMPipeline::reset_kv_state() {
    if(m_adapter_controller) {
        for(auto& state: m_model_runner_generate.query_state()) {
            if(!m_adapter_controller->has_state_name(state.get_name())) {
                state.reset();
            }
        }
    } else {
        m_model_runner_generate.reset_state();
    }
}

void HeteroLLMPipeline::finish_chat() {
